...
<div class="flex flex-col border rounded-md shadow-md md:w-full w-full p-0 bg-white border-none">
  ...
  <div class="p-4 grid <md:grid-cols-1 grid-cols-3 gap-2 ">
    ...
    <!-- SBU -->
    <div>
      <FieldSelect :bind="{ disabled: !actionText, clearable:true }" class="w-full !mt-3" :value="values.m_comp_id"
        @input="v=>{
              if(v){
                values.m_comp_id=v
              }else{
        ...
        displayField="name" :api="{
          ...
          }" placeholder="Pilih SBU" label="SBU" fa-icon="sort-desc" :check="false" />
    </div>

    <!-- SUB -->
    <div>
      <FieldSelect :bind="{ disabled: !actionText || !values.m_comp_id,  clearable:true }" class="w-full !mt-3"
        ...
        displayField="name" :api="{
          ...
          }" placeholder="Pilih SUB" label="SUB" fa-icon="sort-desc" :check="false" />
    </div>

    <!-- CABANG -->
    <div>
      <FieldSelect :bind="{ disabled: !actionText || !values.m_subcomp_id , clearable:true }" class="w-full !mt-3"
        ...
        displayField="name" :api="{
                ...
                params: {
                  ...
                  transform:false,
                  join:false,
                  where: `this.is_active='true' AND this.m_comp_id='${values.m_comp_id}' AND this.m_subcomp_id='${values.m_subcomp_id}'`
                }
          }" placeholder="Pilih Cabang" label=" Cabang " fa-icon="sort-desc" :check="false" />
    </div>

    <!-- DIVISI -->
    <div>
      <FieldSelect :bind="{ disabled: !actionText || !values.m_branch_id , clearable:true }" class="w-full !mt-3"
    ...
    <!-- JABATAN -->
    <div>
      <FieldSelect :bind="{ disabled: !actionText}" class="w-full !mt-3" :value="values.m_posisi_id"
        @input="v => values.m_posisi_id = v" :errorText="formErrors.m_posisi_id ? 'failed' : ''"
        :hints="formErrors.m_posisi_id" label="Jabatan" placeholder="Pilih Jabatan" valueField="id" displayField="name" :api="{
    ...
    <div>
      <FieldSelect class="w-full !mt-3" label="Zona" placeholder="Pilih Zona"
        :bind="{ disabled: !actionText, clearable:false }" :value="values.m_zona_id" :check="false" @input="(v)=>{
              //$log(v)
              values.m_zona_id=v
              //$log(values.m_zona_id)
            }" :errorText="formErrors.m_zona_id?'failed':''" :hints="formErrors.m_zona_id" displayField="nama"
        valueField="id" :api="{
    ...
    <table class="w-full overflow-x-auto mt-2">
      <thead>
        <tr class="border-y">
          ...
          <td class="text-black font-bold text-capitalize px-2 text-left w-[20%]">Tipe
            <label class="text-red-500 space-x-0 pl-0">*</label>
          </td>
          <td class="text-black font-bold text-capitalize px-0 text-left w-[30%]">Total Biaya
            <label class="text-red-500 space-x-0 pl-0">*</label>
          </td>
      ...
      <tbody>
        <tr v-for="a,i in trx_dtl.items" :key="a._id">
          ...
          <td class="text-center px-1">
            <FieldSelect class="w-full !mt-0" :bind="{ disabled: !actionText, clearable:false }"
              :value="trx_dtl.items[i].tipe_id" :check="false" @input="(v)=>{
                        trx_dtl.items[i].tipe_id=v
                      }" @update:valueFull="(v)=>{
                        trx_dtl.items[i].tipe=v
                      }" displayField="value" valueField="id" :api="{
          ...
          <td class="text-right px-1">
            <div class="flex items-center">
              <FieldNumber
                ...
                :value="trx_dtl.items[i].total_biaya" @input="(v)=>trx_dtl.items[i].total_biaya=v" :check="false"
                class="p-0 !m-0" />
          ...
          <td class="text-center px-1">
            <FieldX :bind="{ readonly: !actionText }" class="w-full py-2 !mt-0" :value="trx_dtl.items[i].keterangan"
              @input="v=>trx_dtl.items[i].keterangan=v" :check="false" type="textarea" label=""
          ...
          <table class="w-full overflow-x-auto mt-2">
            <thead>
              <tr class="border-y">
                ...
                <td class="text-black font-bold text-capitalize px-0 text-left w-[15%]">Jenis Transportasi
                  <label class="text-red-500 space-x-0 pl-0">*</label>
                </td>
                <td class="text-black font-bold text-capitalize px-2 text-left w-[20%]">Transportasi
                  <label class="text-red-500 space-x-0 pl-0">*</label>
                </td>
                <td class="text-black font-bold text-capitalize px-2 text-left w-[20%]">Biaya Transportasi
                  <label class="text-red-500 space-x-0 pl-0">*</label>
                </td>
            ...
            <tbody>
              <tr v-for="(a,i) in trx_dtl_sub.items" :key="a.id">
                ...
                <td class="text-center">
                  <FieldSelect class="w-full py-1 !mt-0" :bind="{ disabled: !actionText, clearable:false }"
                    :value="trx_dtl_sub.items[i].jenis_transport_id" @input="(v)=>{
                            trx_dtl_sub.items[i].jenis_transport_id=v
                          }" :check="false" displayField="value" valueField="id" :api="{
                              url: `${store.server.url_backend}/operation/m_general`,
                              headers: {
                                //'Content-Type': 'Application/json',
                                Authorization: `${store.user.token_type} ${store.user.token}`
                ...
                <td class="text-center">
                  <FieldX :bind="{ readonly: !actionText }" class="w-full py-2 !mt-0"
                    :value="trx_dtl_sub.items[i].nama_transport" @input="v=>trx_dtl_sub.items[i].nama_transport=v"
                    :check="false" type="text" label="" placeholder="Nama Transportasi" />
                </td>
                <td class="text-center">
                  <FieldNumber :bind="{ readonly: !actionText}" :value="trx_dtl_sub.items[i].biaya_transport" @input="(v)=>{
                            trx_dtl_sub.items[i].biaya_transport=v
                            countSub()
                          }" :check="false" class="p-0 !m-0" />
                </td>
                <td class="text-center">
                  <FieldX :bind="{ readonly: !actionText }" class="w-full py-2 !mt-0"
                    :value="trx_dtl_sub.items[i].keterangan" @input="v=>trx_dtl_sub.items[i].keterangan=v"
                    :check="false" type="textarea" label="" placeholder="Keterangan" />
                </td>
  ...
  <div class="flex flex-row items-center justify-end space-x-2 p-2">
    ...
    <button
        ...
        v-show="actionText && (data?.can_create || data?.can_update)"
        @click="onSave"
      >
        <icon fa="save" />
        Simpan
