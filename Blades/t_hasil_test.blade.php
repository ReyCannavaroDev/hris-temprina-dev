    ...
    <div class="flex justify-between items-center px-2.5 py-1">
      <div class="flex flex-col md:flex-row md:items-center gap-y-2 md:gap-y-0 gap-x-4">
        ...
        <div class="block md:hidden">
          <select @change="onStatusChange" class="border rounded-md text-sm py-1 px-2.5 w-full">
        ...
        <div class="hidden md:flex flex-wrap gap-2">
          ...
          <button @click="filterShowData('Pending',1)" :class="activeBtn === 1 
          ...
          <button @click="filterShowData('Proses',2)" :class="activeBtn === 2 
          ...
          <button @click="filterShowData('Half Approved',3)" :class="activeBtn === 3 
          ...
          <button @click="filterShowData('Diterima',4)" :class="activeBtn === 4 
          ...
          <button @click="filterShowData('Tidak Diterima',5)" :class="activeBtn === 5 
  ...
  @verbatim
    <div class="flex flex-col border rounded-md shadow-md md:w-full w-full p-0 bg-white border-none">
      ...
      <div class="p-4 grid <md:grid-cols-1 grid-cols-2 gap-2">
        ...
        <div>
          <FieldPopup class="w-full !mt-3" :bind="{ readonly: !actionText }" :value="values.t_loker_id"
            @input="onLokerChange" :errorText="formErrors.t_loker_id ? 'failed' : ''" :hints="formErrors.t_loker_id"
            ...
            }" placeholder="Pilih Lowongan Kerja" label="Nama Lowongan Kerja" :check="false" :columns="[
        ...
        <div>
          <FieldPopup class="w-full !mt-3" :bind="{ readonly: !actionText || !values.t_loker_id }"
            :value="values.t_pelamar_id" @input="v => values.t_pelamar_id = v"
            :errorText="formErrors.t_pelamar_id ? 'failed' : ''" :hints="formErrors.t_pelamar_id" valueField="id"
            displayField="nama_depan" :api="{
            ...
            label="Nama Pelamar" :check="false" :columns="[
              { headerName: 'No', valueGetter: (params) => params.node.rowIndex + 1, width: 60 },
        ...
        <div>
          <FieldSelect :bind="{ disabled: !actionText, clearable:false }" :value="values.tahapan_id" class="w-full !mt-3"
            @input="v=>values.tahapan_id=v" :errorText="formErrors.tahapan_id?'failed':''" :hints="formErrors.tahapan_id"
        ...
        <div>
          <FieldSelect class="w-full !mt-3"
            :bind="{ disabled: !actionText || ['HALF APPROVED', 'DITERIMA', 'TIDAK DITERIMA'].includes(values.status), clearable:false }"
            :value="values.status" @input="v=>values.status=v" :errorText="formErrors.status?'failed':''"
            :hints="formErrors.status" valueField="value" displayField="value" :options="statusOptions"
      ...
      <div class="grid grid-cols-8 md:grid-cols-12 text-[14px] gap-x-[29px] gap-y-[26px] mx-4">
        ...
        <div class="col-span-8 md:col-span-12">
          <button v-if="actionText" @click="addDetail" type="button"
            ...
            <table class="w-full overflow-x-auto table-auto border border-[#CACACA]">
              ...
              <tbody>
                <tr v-for="(item, i) in detailArr" :key="item.id" class="border-t" v-if="detailArr.length > 0">
                  ...
                  <td class="p-2 border border-[#CACACA]">
                    <FieldSelect :bind="{ disabled: !actionText, clearable: false }" :value="item.nama_tes"
                      @input="v=>item.nama_tes=v" :errorText="formErrors.nama_tes?'failed':''" :hints="formErrors.nama_tes"
                      ...
                      }" placeholder="Pilih Nama Test" label="" fa-icon="" :check="false" />
                  </td>

                  <td class="p-2 border border-[#CACACA]">
                    <FieldX :bind="{ readonly: !actionText }" type="number" step="any" min="0" max="100"
                      class="w-full !mt-0" :value="item.nilai_tes" :errorText="formErrors.nilai_tes?'failed':''"
                      @input="v=>item.nilai_tes=v" :hints="formErrors.nilai_tes" placeholder="Masukkan Nilai Test" label=""
                      fa-icon="" :check="false" />
                  </td>
      ...
      <div class="flex flex-row items-center justify-end space-x-2 p-2">
        ...
        <button
          ...
          v-show="isRead" @click="onPrint()">
          <icon fa="print" />
          Print
        </button>
        <button
          class="bg-red-600 text-white font-semibold hover:bg-red-500 transition-transform duration-300 transform hover:-translate-y-0.5 rounded-md p-2"
          v-show="actionText" @click="onReset(true)">
          <icon fa="times" />
          Reset
        </button>
        <button
          class="bg-green-600 text-white font-semibold hover:bg-green-500 transition-transform duration-300 transform hover:-translate-y-0.5 rounded-md p-2"
          v-show="actionText" @click="onSave">
          <icon fa="save" />
          Simpan
        </button>
      </div>
