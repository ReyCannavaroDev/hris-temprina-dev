...
<div class="flex flex-col border rounded-md shadow-md md:w-full w-full p-0 bg-white border-none">
  ...
  <div class="p-4 grid <md:grid-cols-1 grid-cols-3 gap-2 ">
    ...
    <div>
      <FieldSelect :bind="{ disabled: !actionText, clearable:true }" class="w-full !mt-3" :value="values.m_branch_id"
        ...
        displayField="name" :api="{          
          ...
          }" placeholder="Pilih Cabang" label="Cabang" :check="false" />
    </div>

    <div class="grid grid-cols-12 items-center">
      <div class="col-span-3">
        <label class="inline-block whitespace-nowrap" for="isParent">Parent DIVISI</label>
      </div>
      <span>:</span>
      <div class="flex">
    ...
    <div>
      <FieldPopup v-if="values.is_parent" class="w-full !mt-3" :bind="{ readonly: !actionText }"
        :value="values.parent_id" @input="v=>{
          if(v){
            values.parent_id=v
          }else{
            values.parent_id=null
          ...
          params: {
            simplest:true,
            where: `this.m_branch_id = ${values.m_branch_id ?? 0} AND this.is_active = true`
          }
        }" placeholder="Pilih Parent Divisi" label="Parent Divisi" :check="false" :columns="[{
          headerName: 'No',
    ...
    <div class="grid grid-cols-2 gap-x-4">
      <FieldNumber v-show="values.is_parent" :bind="{ readonly: true }" class="w-full !mt-3" :value="values.nomorParent"
      ...
      <FieldNumber :bind="{ readonly: !actionText }" :class="{'col-span-2':!values.is_parent}" class="w-full !mt-3"
    ...
    <div>
      <FieldSelect :bind="{ disabled: !actionText, clearable:true }" class="w-full !mt-3" :value="values.name" @input="v=>{
            if(v){
              values.name=v
            }else{
              values.name=null
            ...
            values.nomorParent = null
            values.level = 0
          }" :errorText="formErrors.name?'failed':''" :hints="formErrors.name" valueField="id" displayField="value"
        :api="{          
              url: `${store.server.url_backend}/operation/m_general`,
              headers: { 'Content-Type': 'Application/json', Authorization: `${store.user.token_type} ${store.user.token}`},
              params: {
                simplest:true,
                where: `this.group ='MASTER_DIVISI' AND this.is_active = true`,
              }
    ...
    <!-- <div class="relative">
      <FieldX :bind="{ readonly: !actionText }" class="w-full !mt-3" :value="values.name"
        ...
        label="Nama" placeholder="Masukkan atau Pilih Nama" />
