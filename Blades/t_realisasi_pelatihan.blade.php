...
<div class="flex flex-col border rounded-md shadow-md md:w-full w-full p-0 bg-white border-none">
  <div class="bg-gray-500 text-white rounded-t-md py-2 px-4">
    <div class="flex items-center">
      <Icon fa="arrow-left" class="cursor-pointer mr-2 font-bold hover:text-yellow-500" title="Kembali"
        @click="onBack" />
      <div>
        <h1 class="text-20px font-bold">Form Realisasi Pelatihan</h1>
        <p class="text-gray-100">Realisasi Pelatihan</p>
      </div>
    </div>
  </div>
      ...
      <table class="w-full overflow-x-auto table-auto border border-[#CACACA]">
        ...
        <tbody>
          <tr v-for="(item, i) in detailArr" :key="item.id || item.m_kary_id || i" class="border-t" v-if="detailArr.length > 0">
            ...
            <td class="text-center border border-[#CACACA]">
              <FieldSelect class="mt-0 w-full" :bind="{ disabled: true, clearable:false }" :value="item.m_kary_id"
                @input="v=>item.m_kary_id=v" :errorText="formErrors.m_kary_id?'failed':''" :hints="formErrors.m_kary_id"
            ...
            <td class="text-center border border-[#CACACA]">
              <FieldSelect class="mt-0 w-full" :bind="{ disabled: true, clearable:false }" :value="item.m_branch_id"
                @input="v=>item.m_branch_id=v" :errorText="formErrors.m_branch_id?'failed':''"
            ...
            <td class="text-center border border-[#CACACA]">
              <FieldSelect class="mt-0 w-full" :bind="{ disabled: true, clearable:false }" :value="item.m_posisi_id"
  ...
  <div class="flex flex-row items-center justify-end space-x-2 p-2">
    ...
    <button
        ...
        v-show="actionText && currentMenu.can_create"
        @click="onSave"
      >
        <icon fa="save" />
        Simpan
