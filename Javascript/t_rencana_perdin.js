  ...
  if (isRead) {
    ...
    try {
      ...
      if (route.query.is_approval) {
        ...
        // initialValues.t_rencana_perdin_det.forEach((dt) => {
...
function onProcess(typePar) {
  let next = true
  // if(typePar?.toLowerCase() === 'revise' ||typePar?.toLowerCase() === 'reject'){
  //   if(!values.note_approval){
  //     swal.fire({
  //         icon: 'warning',
  //         text: `Catatan approval diperlukan!`
  //       })
  //       next = false
  //       return
  //   }
  // }
  const requiredTypes = ['revise', 'reject'];

  if (requiredTypes.includes(typePar?.toLowerCase())) {
    if (!values.note_approval) {
      swal.fire({
        icon: 'warning',
        text: 'Catatan approval diperlukan!'
    ...
    if (res.isConfirmed) {
      try {
        ...
        if (!res.ok) {
          ...
          router.replace('/notification?reload=true');
        }
      } catch (err) {
        isBadForm.value = true;
        swal.fire({
...
const landing = reactive({
  actions: [
    ...
    },
    {
      icon: 'edit',
      title: "Edit",
      show: (row) => !['POST', 'APPROVAL', 'APPROVED', 'REJECTED'].includes(row['status']),
  ...
  },
  {
    ...
    cellRenderer: (params) => {
      return params.data['status']?.toUpperCase() == 'DRAFT'
        ...
        : (params.data['status']?.toUpperCase() == 'POST' ? `<span class="text-amber-600 rounded-md text-xs font-medium px-4 py-1 inline-block capitalize">${params.data['status']?.toUpperCase()}</span>`
          : (params.data['status']?.toUpperCase() == 'APPROVAL' ? `<span class="text-blue-600 rounded-md text-xs font-medium px-4 py-1 inline-block capitalize">${params.data['status']?.toUpperCase()}</span>`
            : (params.data['status']?.toUpperCase() == 'APPROVED' ? `<span class="text-green-600 rounded-md text-xs font-medium px-4 py-1 inline-block capitalize">${params.data['status']?.toUpperCase()}</span>`
              : (params.data['status']?.toUpperCase() == 'REJECTED' ? `<span class="text-purple-600 rounded-md text-xs font-medium px-4 py-1 inline-block capitalize">${params.data['status']?.toUpperCase()}</span>`
                : (params.data['status']?.toUpperCase() == 'REVISED' ? `<span class="text-red-600 rounded-md text-xs font-medium px-4 py-1 inline-block capitalize">${params.data['status']?.toUpperCase()}</span>`
