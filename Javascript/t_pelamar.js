...
onBeforeMount(async () => {
  try {
      ...
      try {
        ...
        const assignDetails = (key, target, idRef) => {
          initialValues[key]?.forEach(async (item) => {
            if (key === 't_pelamar_det_org' || key === 't_pelamar_det_pres' || key === 't_pelamar_det_pel' || key === 't_pelamar_det_kel' || key === 't_pelamar_det_pend') {
        ...
        assignDetails('t_pelamar_d_lokasi', detail_lokasi, { value: 0 })
        assignDetails('t_pelamar_det_pend', detailPendidikan, { value: _idPend })
        assignDetails('t_pelamar_det_pel', detailPelatihan, { value: _idPel })
        assignDetails('t_pelamar_det_pres', detailPrestasi, { value: _idPres })
        assignDetails('t_pelamar_det_org', detailOrganisasi, { value: _idOrg })
        assignDetails('t_pelamar_det_bhs', detailBahasa, { value: _idBhs })
        ...
        console.log('inDetailArr:', inDetailArr.value)
    ...
    if (values.t_pelamar_det_pemb?.length > 0) {
      const pemb = values.t_pelamar_det_pemb[0]
    ...
    if (values.t_pelamar_det_kartu?.length > 0) {
      const kartu = values.t_pelamar_det_kartu[0]
      Object.assign(values, {
        ktp_no: kartu.ktp_no,
        kk_no: kartu.kk_no,
...
async function imageChange(e) {
  ...
  if (file[0]) {
    ...
    if (e.target.id === 'inputPasFoto') {
      ...
      // console.log(file[0])
      const res = await fetch(`${store.server.url_backend}/operation/t_pelamar_det_kartu/upload` + '?' + `field=pas_foto`, {
        method: 'POST',
        headers: {
          Authorization: `${store.user.token_type} ${store.user.token}`
        },
      ...
      var formData = new FormData()
      formData.append('file', file[0])
      const res = await fetch(`${store.server.url_backend}/operation/t_pelamar_det_kartu/upload` + '?' + `field=ktp_foto`, {
        method: 'POST',
        headers: {
          Authorization: `${store.user.token_type} ${store.user.token}`
        },
      ...
      var formData = new FormData()
      formData.append('file', file[0])
      const res = await fetch(`${store.server.url_backend}/operation/t_pelamar_det_kartu/upload` + '?' + `field=kk_foto`, {
        method: 'POST',
        headers: {
          Authorization: `${store.user.token_type} ${store.user.token}`
        },
      ...
      const res = await fetch(`${store.server.url_backend}/operation/t_pelamar_det_kartu/upload` + '?' + `field=npwp_foto`, {
        method: 'POST',
        headers: {
          Authorization: `${store.user.token_type} ${store.user.token}`
        },
      ...
      const res = await fetch(`${store.server.url_backend}/operation/t_pelamar_det_kartu/upload` + '?' + `field=bpjs_foto`, {
        method: 'POST',
        headers: {
          Authorization: `${store.user.token_type} ${store.user.token}`
        },
...
async function fileIjazah(e) {
  ...
  if (file[0]) {
    ...
    var formData = new FormData()
    formData.append('file', file[0])
    const res = await fetch(`${store.server.url_backend}/operation/t_pelamar_det_pend/upload` + '?' + `field=ijazah_foto`, {
      method: 'POST',
      headers: {
        Authorization: `${store.user.token_type} ${store.user.token}`
      },
...
async function fileSrtRef(e) {
  ...
  if (file[0]) {
    ...
    var formData = new FormData()
    formData.append('file', file[0])
    const res = await fetch(`${store.server.url_backend}/operation/t_pelamar_det_pk/upload` + '?' + `field=surat_referensi`, {
      method: 'POST',
      headers: {
        Authorization: `${store.user.token_type} ${store.user.token}`
      },
...
async function onSave() {
  ...
  values.t_pelamar_det_dokumen = detail_Jabatan
  values.t_pelamar_det_jobdesc = detail_Jobdesk.length ? detail_Jobdesk : []
  values.t_pelamar_d_lokasi = detail_lokasi
  values.nama_lengkap = `${values.nama_depan ?? ''} ${values.nama_belakang ?? ''}`.trim()
  values.t_pelamar_det_pres = detailPrestasi.value
  values.t_pelamar_det_kel = detailKeluarga.value
  values.t_pelamar_det_org = detailOrganisasi.value
  values.t_pelamar_det_bhs = detailBahasa.value
  values.t_pelamar_det_pend = detailPendidikan.value
  values.t_pelamar_det_pel = detailPelatihan.value
  values.t_pelamar_det_pk = detailPengalaman.value

  if (values.periode_gaji_id) {
    values.t_pelamar_det_pemb = [{
  ...
  if (isCreating || values.t_pelamar_det_kartu?.length === 0) {
    values.t_pelamar_det_kartu = [{
    ...
    const kartu = values.t_pelamar_det_kartu[0]
    const init = initialValues.t_pelamar_det_kartu[0] || {}
...
async function handleCvUpload(file) {
  ...
  cvScanModal.open = true
  cvScanModal.fileName = file.name
  cvScanModal.fileSize = formatFileSize(file.size)
  cvScanModal.progress = 15
  cvScanModal.status = 'scanning'
  cvScanModal.currentStep = 1
  cvScanModal.errorMessage = ''
  cvScanModal.summary = {
    nama: '-',
    email: '-',
    telp: '-',
  ...
  // Simulasi langkah visual ekstraksi
  const stepTimer1 = setTimeout(() => {
    if (cvScanModal.status === 'scanning') {
      cvScanModal.currentStep = 2
      cvScanModal.progress = 35
    }
  }, 400)

  const stepTimer2 = setTimeout(() => {
    if (cvScanModal.status === 'scanning') {
      cvScanModal.currentStep = 3
      cvScanModal.progress = 60
    }
  }, 800)

  const stepTimer3 = setTimeout(() => {
    if (cvScanModal.status === 'scanning') {
      cvScanModal.currentStep = 4
      cvScanModal.progress = 80
    }
  }, 1200)
  ...
  try {
    ...
    // Step 5: Auto-fill ke formulir
    cvScanModal.currentStep = 5
    cvScanModal.progress = 95
    ...
    // Sukses & Selesai
    setTimeout(() => {
      cvScanModal.progress = 100
      cvScanModal.status = 'success'
      cvScanModal.summary = {
        nama: data.nama_lengkap || data.nama_depan || '-',
        email: data.email || '-',
        telp: data.telp || '-',
        pendidikanCount: countPend,
        pengalamanCount: countPk,
        fileCv: data.file_cv || ''
      }
    }, 400)
    ...
    cvScanModal.status = 'error'
    cvScanModal.errorMessage = err.message || 'Terjadi kesalahan saat memproses file CV.'
        ...
        click(row) {
          swal.fire({
            ...
            if (result.isConfirmed) {
              try {
                const dataURL = `${store.server.url_backend}/operation${endpointApi}/?id=${row.id}`
                isRequesting.value = true
                const res = await fetch(dataURL, {
                  method: 'POST',
                  headers: {
