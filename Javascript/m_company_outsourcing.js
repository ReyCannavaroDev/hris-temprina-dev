...
// ------------------------------ PERSIAPAN
const endpointApi = 'm_company_outsourcing'
onBeforeMount(()=>{
  document.title = 'Master Company Outsourcing'
})
...
function preventInvalidInput(event) {
  ...
  if (allowedKeys.includes(event.key) || !isNaN(Number(event.key))) {
    formErrors.value["number"] = null;
    return;
  }
  event.preventDefault();
  formErrors.value["number"] = ['Hanya dapat menginputkan angka'];
}

function npwp_key(event) {
  const allowedKeys = ['Backspace', 'Delete', 'Tab', 'ArrowLeft', 'ArrowRight', '.', '-'];
  if (event.ctrlKey || event.metaKey) {
    return;
  }
  ...
  event.preventDefault();
  formErrors.value["npwp"] = ['Hanya angka, ".", dan "-" yang diperbolehkan'];
}
...
function validateWebsite(v) {
  // Validasi URL dengan HTTP, HTTPS, atau WWW
  const urlRegex = /^(https?:\/\/)?(www\.)?[a-zA-Z0-9-]+\.[a-zA-Z]{2,6}(\.[a-zA-Z]{2,6})?(\/\S*)?$/;
  ...
  if (!v) {
    ...
    formErrors.value["website"] = ['Please enter a valid website address (e.g., https://www.example.com)'];
    web.value = false;
  }
  ...
  //values.tags = JSON.stringify(values.tags)
    try {
      ...
      swal.fire({
        ...
        text: 'Harap periksa format email Anda.',
      });
      return; 
    }
    ...
    if (!web.value) {
      swal.fire({
        ...
        text: 'Harap masukkan alamat website yang valid.',
      });
      return;
    }
...
const landing = reactive({
  ...
  api: {
    ...
    params: {
      join:true,
      simplest: true,
      searchfield: 'this.code, this.name, this.address, city.value1, prov.value1, this.is_active'
    },
    onsuccess(response) {
