...
import { ref, readonly, reactive, inject, onMounted, onBeforeMount, watchEffect, watch, onActivated, computed } from 'vue'
...
function onProcess(typePar) {
  ...
  swal.fire({
    ...
    if (res.isConfirmed) {
      try {
        const dataURL = `${store.server.url_backend}/operation/t_klaim_askes/progress`;
...
//       console.log(`Row ${index + 1} nominal:`, item.nominal)
//     })
//   },
//   { deep: true }
// )
  ...
  (newVal) => {
    ...
    newVal.forEach((item) => {
      ...
      if (santunan === 100) {
        ...
        accepted = Math.round(nominal * (santunan / 100))
      }
      else {
        accepted = Math.round(nominal * (klaimTable / 100))
      }

      if (item.accepted !== accepted) {
        item.accepted = accepted
        item.reject = nominal - accepted
      }
    ...
    if (values.total_nominal !== totalAccepted) {
      values.total_nominal = totalAccepted
    }
...
async function fetchPlafond() {
  try {
    const res = await fetch(
      `${store.server.url_backend}/operation/m_kary/getplafond`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    if (!res.ok) {
      ...
      throw errJson.message || 'Gagal mengambil plafond'
    }

    const data = await res.json()
    console.log(data)
    values.plafond = data.plafond
    values.sisa_plafond = data.sisa_plafond
...
const apiAskes = computed(() => {
  return {
    url: `${store.server.url_backend}/operation/m_kary/getAskes`,
    headers: {
      'Content-Type': 'application/json',
      Authorization: `${store.user.token_type} ${store.user.token}`,
    },
    ...
    onsuccess: (response) => {
      ...
      const transformed = rawData.map(item => {
        if (item.klaim_type === 'm_kary') {
          item.klaim_table = '80'
        } else if (item.klaim_type === 'm_kary_det_kel') {
          item.klaim_table = '70'
        }
        return item
      })
...
async function onSave() {
  try {
    // values.nomor = '1'
    values.t_klaim_askes_d = detailArr.value
      ...
      ,
      {
        ...
        async click(row) {
          swal.fire({
            ...
            if (res.isConfirmed) {
              try {
                const dataURL = `${store.server.url_backend}/operation/t_klaim_askes/approveHC`
                isRequesting.value = true
                const res = await fetch(dataURL, {
                  method: 'POST',
                  headers: {
