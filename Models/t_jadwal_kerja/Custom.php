...
{    
    ...
    public function scopeDateRange($model){ 
        ...
        return $model->whereBetween('start_date', [$date_from, $date_from]);
    }

    public function scopeGetJamKerja($model){
        return $model->with(['t_jadwal_kerja_det_hari'=> function($query){
            $query->join('m_general', 'm_general.id', 't_jadwal_kerja_det_hari.m_jam_kerja_id');
        }]);
    }
}