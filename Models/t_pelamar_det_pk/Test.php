<?php
namespace Tests;
use Laravel\Lumen\Testing\DatabaseTransactions;
use Laravel\Passport\Passport;
use App\Models\Defaults\User;

class tPelamarDetPkTest extends TestCase
{
    use DatabaseTransactions;

    public function testReadingData()
    {
        Passport::actingAs(User::first());
        $payload = [
            'paginate' => 25
        ];

        $this->call('GET', '/operation/t_pelamar_det_pk', $payload);

        $responseArr = json_decode( $this->response->getContent(),true );
        ff( $responseArr, 'dump data' );

        $this->assertTrue(true);
    }

    public function testCreatingData()
    {
        $user = User::where('username', 'USERNAME')->first();
        $this->assertNotEmpty( $user );
        
        Passport::actingAs($user);

        $payload = [
		    "id" => "bigint:optional:autocreate",
		    "t_pelamar_id" => "bigint:optional",
		    "instansi" => "string:100:required",
		    "bidang_usaha" => "string:100:required",
		    "no_tlp" => "string:20:required",
		    "posisi" => "string:100:required",
		    "thn_masuk" => "integer:required",
		    "thn_keluar" => "integer:required",
		    "alamat_kantor" => "text:required",
		    "kota_id" => "bigint:required",
		    "surat_referensi" => "string:191:required",
		    "creator_id" => "integer:optional",
		    "last_editor_id" => "integer:optional",
		    "created_at" => "datetime:optional:autocreate",
		    "updated_at" => "datetime:optional:autocreate"
		];

        $this->call('POST', '/operation/t_pelamar_det_pk', $payload);

        $responseArr = json_decode( $this->response->getContent(),true );
        // ff( $responseArr, 'dump data' );

        $this->assertEquals( 200, $this->response->status() );
        // $this->seeJsonStructure( ['status'] );

        $this->seeInDatabase('t_pelamar_det_pk', array_filter($payload, function($dt){
            return !is_array($dt);
        } ));
    }
}