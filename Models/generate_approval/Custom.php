...
{    
    ...
    public function generate_approval_d()
    {
        return $this->hasMany(generate_approval_d::class);
    }
    ...
    public function custom_outstanding($req)
    {
    ...
    //     $filteredData = $outstanding->getCollection()->filter(function($item) use ($m_branch_id, $m_subcomp_id) {

    //         $branchMatch = is_null($m_branch_id)
    //             ? true
    //             : ($item->m_branch_id ?? null) == $m_branch_id;

    //         $subcompMatch = is_null($m_subcomp_id)
    //             ? true
    //             : ($item->m_subcomp_id ?? null) == $m_subcomp_id;

    //         return $branchMatch && $subcompMatch;

    //     })->values();

    //     $outstanding->setCollection($filteredData);
