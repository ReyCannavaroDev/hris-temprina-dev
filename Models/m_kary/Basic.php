...
{   
    ...
    public $columnsFull = ["id:bigint","m_comp_id:bigint","m_dir_id:bigint","m_divisi_id:bigint","m_dept_id:bigint","m_zona_id:bigint","grading_id:bigint","costcontre_id:bigint","kode:string:100","m_posisi_id:bigint","m_jam_kerja_id:json","kode_presensi:string:100","nik:string:20","nama_depan:string:100","nama_belakang:string:100","nama_lengkap:string:100","nama_panggilan:string:100","jk_id:bigint","tempat_lahir:string:100","tgl_lahir:date","provinsi_id:bigint","kota_id:bigint","kecamatan_id:bigint","kode_pos:string:10","alamat_asli:text","alamat_domisili:text","no_tlp:string:20","no_tlp_lainnya:string:20","no_darurat:string:20","nama_kontak_darurat:string:100","agama_id:bigint","gol_darah_id:bigint","status_nikah_id:bigint","tanggungan_id:bigint","hub_dgn_karyawan:string:100","cuti_jatah_reguler:integer","cuti_sisa_reguler:integer","cuti_panjang:integer","cuti_sisa_panjang:integer","status_kary_id:bigint","lama_kontrak_awal:date","lama_kontrak_akhir:date","tgl_masuk:date","tgl_berhenti:date","alasan_berhenti:text","uk_baju:string:50","uk_celana:string:50","uk_sepatu:string:50","desc:text","is_active:boolean","creator_id:bigint","last_editor_id:bigint","created_at:datetime","updated_at:datetime","m_standart_gaji_id:bigint","periode_gaji_id:bigint","ref_id:bigint","presensi_lokasi_default_id:bigint","exp_date_cuti:date","limit_potong:integer","atasan_id:bigint","cuti_p24:decimal","cuti_sisa_p24:decimal","tipe_jam_kerja_id:bigint","t_jadwal_kerja_id:bigint","ig:string:100","x:string:100","facebook:string:100","linkedin:string:100","email:string:100","m_fingerprint_machine_id:bigint","m_company_outsourcing_id:bigint","no_registrasi:string:50","m_subcomp_id:bigint","m_branch_id:bigint","finger_id:string:191","cuti_jatah_tahun_lalu:bigint","nip:string:50","is_sync:boolean","can_outscope:boolean","tgl_pengangkatan:date"];
    public $rules       = [];
    public $joins       = ["m_comp.id=m_kary.m_comp_id","m_dir.id=m_kary.m_dir_id","m_divisi.id=m_kary.m_divisi_id","m_dept.id=m_kary.m_dept_id","m_zona.id=m_kary.m_zona_id","m_general.id=m_kary.grading_id","m_general.id=m_kary.costcontre_id","m_posisi.id=m_kary.m_posisi_id","m_jam_kerja.id=m_kary.m_jam_kerja_id","m_general.id=m_kary.jk_id","m_general.id=m_kary.provinsi_id","m_general.id=m_kary.kota_id","m_general.id=m_kary.kecamatan_id","m_general.id=m_kary.agama_id","m_general.id=m_kary.gol_darah_id","m_general.id=m_kary.status_nikah_id","m_general.id=m_kary.tanggungan_id","m_general.id=m_kary.status_kary_id","default_users.id=m_kary.creator_id","default_users.id=m_kary.last_editor_id","m_standart_gaji.id=m_kary.m_standart_gaji_id","m_general.id=m_kary.periode_gaji_id","t_pelamar.id=m_kary.ref_id","presensi_lokasi.id=m_kary.presensi_lokasi_default_id","m_kary.id=m_kary.atasan_id","m_general.id=m_kary.tipe_jam_kerja_id","t_jadwal_kerja.id=m_kary.t_jadwal_kerja_id","m_fingerprint_machine.id=m_kary.m_fingerprint_machine_id","m_company_outsourcing.id=m_kary.m_company_outsourcing_id","m_subcomp.id=m_kary.m_subcomp_id","m_branch.id=m_kary.m_branch_id"];
    public $details     = ["m_kary_det_org","m_kary_det_pel","m_kary_det_kel","m_kary_d_lokasi","m_kary_det_bhs","m_kary_det_jobdesc","m_kary_det_kartu","m_kary_det_pemb","m_kary_det_pend","m_kary_det_pk","m_kary_det_pres","m_kary_det_jabatan"];
    public $heirs       = ["t_rencana_perdin","t_perdin","t_penyelesaian_perdin","default_users","t_work_log","t_work_log","t_work_log","t_work_log","t_work_log","t_final_gaji_det","t_mutasi","t_mutasi","t_perhitungan_gaji","m_kary","t_assessment_kary","t_assessment_kary","t_cuti","m_standart_gaji","t_cuti_adjustment","t_jadwal_kerja_det","t_lembur","t_potongan","t_riwayat_posisi","t_salary_borongan","t_sgp","t_spd"];
    public $detailsChild= [];
    public $detailsHeirs= [];
    public $unique      = [];
    ...
    public function m_kary_d_lokasi() :\HasMany
    {
        return $this->hasMany('App\Models\BasicModels\m_kary_d_lokasi', 'm_kary_id', 'id');
    }
    public function m_kary_det_bhs() :\HasMany
    {
        return $this->hasMany('App\Models\BasicModels\m_kary_det_bhs', 'm_kary_id', 'id');
    }
    public function m_kary_det_jobdesc() :\HasMany
    {
        return $this->hasMany('App\Models\BasicModels\m_kary_det_jobdesc', 'm_kary_id', 'id');
    }
    ...
    public function m_kary_det_jabatan() :\HasMany
    {
        return $this->hasMany('App\Models\BasicModels\m_kary_det_jabatan', 'm_kary_id', 'id');
    }
    ...
    public function m_fingerprint_machine() :\BelongsTo
    {
        return $this->belongsTo('App\Models\BasicModels\m_fingerprint_machine', 'm_fingerprint_machine_id', 'id');
    }
    public function m_company_outsourcing() :\BelongsTo
    {
        return $this->belongsTo('App\Models\BasicModels\m_company_outsourcing', 'm_company_outsourcing_id', 'id');
    }
